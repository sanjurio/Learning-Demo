...
def index():
    ...
    return render_template('index.html', title='Welcome to AI Learning Platform')
...
def setup_2fa():
    ...
    return render_template('auth/two_factor.html', title='Setup Two-Factor Authentication', 
                         form=form, qr_code=qr_code, setup=True)
...
def view_course(course_id):
    ...
    return render_template('user/course.html', title=course.title, course=course, lessons=lessons)
...
def view_lesson(lesson_id):
    ...
    return render_template('user/lesson.html', title=lesson.title, 
...
def user_interests():
    ...
    return render_template('user/interests.html', title='My Interests', 
                         form=form, interest_status=interest_status)
...
def admin_courses():
    ...
    return render_template('admin/content.html', title='Manage Courses', courses=courses)
...
def admin_add_course():
    ...
    return render_template('admin/edit_course.html', title='Add Course', form=form, course=None)
...
def admin_lessons(course_id):
    ...
    return render_template('admin/lessons.html', title='Manage Lessons', course=course, lessons=lessons)
...
def admin_add_lesson(course_id):
    ...
    return render_template('admin/edit_lesson.html', title='Add Lesson', form=form, course=course, lesson=None)
...
def admin_edit_lesson(lesson_id):
    ...
    return render_template('admin/edit_lesson.html', title='Edit Lesson', form=form, course=course, lesson=lesson)
...
def admin_interests():
    ...
    interests = Interest.query.all()
    return render_template('admin/interests.html', title='Manage Interests', interests=interests)
...
def admin_add_interest():
    ...
    return render_template('admin/edit_interest.html', title='Add Interest', form=form, interest=None)
...
def admin_user_interests(user_id):
    ...
    return render_template('admin/user_interests.html', title=f'Manage Access for {user.username}',
                         user=user, interest_status=interest_status, form=form)
...
def test_openai_connection():
    ...
    try:
        ...
        if openai_key:
            app.logger.info(f"Found OpenAI API key in database: {openai_key.key_value[:4]}...{openai_key.key_value[-4:]}")
        else:
            app.logger.warning("No OpenAI API key found in database")
        
        # Set from database if needed
        if openai_key and not env_key:
            app.logger.info("Setting OpenAI API key from database to environment")
            os.environ['OPENAI_API_KEY'] = openai_key.key_value
            env_key = openai_key.key_value
        ...
        if not env_key and not openai_key:
            return jsonify({
                'success': False,
                'message': 'No OpenAI API key found in environment or database',
                'details': {
                    'env_key': False,
                    'db_key': False
                }
            ...
            if hasattr(client, 'chat') and hasattr(client.chat, 'completions'):
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": "Hello, this is a test message. Please respond with 'API connection is working'."}
                    ],
                    max_tokens=20
                )
...
def api_analyze_document():
    ...
    try:
        ...
        app.logger.info(f"Analysis result success: {result.get('success', False)}")
